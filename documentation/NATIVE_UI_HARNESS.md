...
devdrivr running alongside the dev one).
...
screenshot to `%TEMP%\devdrivr-native-ui\`. Exit code is 1 if anything failed.
...
failure. `Wait-devdrivrWindowState` polls with a timeout.
...
**`process "devdrivr"`, not `process "devdrivr"`.** `devdrivr` is the window title and also appears
...
devdrivr persists everything through `plugin:sql`, so the app kept rendering perfectly while
