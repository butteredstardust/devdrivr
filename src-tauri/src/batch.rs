//! Atomic multi-statement writes against the devdrivr database.
//!
...
fn devdrivr_db_path(app: &AppHandle) -> Result<PathBuf, String> {
...
impl BatchDb {
    async fn pool(&self, app: &AppHandle) -> Result<SqlitePool, String> {
        ...
        let db_path = devdrivr_db_path(app)?;
        ...
        let pool = SqlitePoolOptions::new()
            ...
            .await
            .map_err(|err| format!("Failed to open devdrivr database for batch write: {err}"))?;
