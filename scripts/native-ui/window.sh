...
#   ./window.sh solo          # fail unless exactly one devdrivr/devdrivr process is running
...
#
# The process name is NOT the product name: `devdrivr` is the process, `devdrivr` is the *window*
...
case "${1:-}" in
  ...
  solo)
    ...
    running=$(osascript -e 'tell application "System Events" to return name of every process whose name is "devdrivr" or name is "devdrivr"')
    count=$(printf '%s' "$running" | awk -F', ' '{print NF}')
    if [ "$running" = "" ]; then
      echo "no devdrivr process running" >&2
      exit 3
