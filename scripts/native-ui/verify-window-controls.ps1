...
.PARAMETER ProcessName
  Override the process name (default `devdrivr`, the dev binary; use `devdrivr` for a packaged build).
#>
[CmdletBinding()]
param(
  [switch]$SkipClose,
  [string]$ShotDirectory = (Join-Path ([IO.Path]::GetTempPath()) 'devdrivr-native-ui'),
...
function Save-FailureShot {
  ...
  try {
    ...
    $path = Save-DevdrivrScreenshot -Window $window -Path (Join-Path $ShotDirectory "fail-$slug.png")
...
function Test-Control {
  ...
  try {
    $window = Get-DevdrivrWindow -ProcessName $ProcessName
    $point = Get-DevdrivrControlPoint -Window $window -Control $Control
    [void](Assert-ControlPointIsClickable -Window $window -Point $point)

    $before = Get-DevdrivrWindowState -Window $window
    ...
    $outcome = Wait-DevdrivrWindowState -State $Expect -TimeoutMs $TimeoutMs -ProcessName $ProcessName
    if ($outcome.Reached) {
...
function Reset-ToNormal {
  ...
  try {
    $window = Get-DevdrivrWindow -ProcessName $ProcessName
    Restore-DevdrivrWindow -Window $window
    $window = Get-DevdrivrWindow -ProcessName $ProcessName
...
try {
  $window = Get-DevdrivrWindow -ProcessName $ProcessName
  $point = Get-DevdrivrControlPoint -Window $window -Control 'dragregion'
  $originX = $window.X
  $originY = $window.Y
  $deltaX = 120
  $deltaY = 60
  ...
  $moved = Get-DevdrivrWindow -ProcessName $ProcessName
