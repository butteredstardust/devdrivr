...
// Static fallback themes — used when the user explicitly picks devdrivr-dark or devdrivr-light
...
// buildDevdrivrTheme() has no palette of its own — every token colour below is
 ...
 */
function buildDevdrivrTheme(isLight: boolean): MonacoThemeData {
  ...
  // devdrivr-native themes' variable sets.
...
export { buildDevdrivrTheme }

function resolveMonacoTheme(appTheme: string, editorTheme: string): string {
  if (editorTheme === 'devdrivr-dark') return 'devdrivr-dark'
  if (editorTheme === 'devdrivr-light') return 'devdrivr-light'
  // match-app: use the pre-built package theme if available, otherwise devdrivr-current
  const pkg = MONACO_PACKAGE_THEMES[appTheme]
  return pkg ? pkg.monacoId : 'devdrivr-current'
}
...
export function useMonacoSettings() {
  ...
  useEffect(() => {
    ...
    loader.init().then(async (monaco) => {
      if (!themesRegistered) {
        monaco.editor.defineTheme('devdrivr-dark', DARK_THEME)
        monaco.editor.defineTheme('devdrivr-light', LIGHT_THEME)
      ...
      if (resolvedTheme === 'devdrivr-current') {
        ...
        monaco.editor.defineTheme(
          'devdrivr-current',
          buildDevdrivrTheme(isLightEffectiveTheme(effective))
        )
      }
