...
// mimic what the real cascade would produce for each devdrivr-native theme
...
describe('buildDevdrivrTheme', () => {
  afterEach(() => {
    vi.restoreAllMocks()
  })

  it('emits a non-empty rules array for a dark devdrivr-native theme, derived from theme variables', () => {
    stubCssVars(MIDNIGHT_VARS)
    const theme = buildDevdrivrTheme(false)
  ...
  it('produces a different rules array for a different theme (proves it is not a constant)', () => {
    stubCssVars(MIDNIGHT_VARS)
    const midnightTheme = buildDevdrivrTheme(false)

    stubCssVars(SOFT_FOCUS_VARS)
    const softFocusTheme = buildDevdrivrTheme(true)
  ...
  it('keeps every derived token colour at ~3:1 or better contrast against --color-surface (light theme)', () => {
    stubCssVars(SOFT_FOCUS_VARS)
    const theme = buildDevdrivrTheme(true)
    const bg = theme.colors['editor.background']!
  ...
  it.each([
    ...
    (_label, vars, isLight) => {
      stubCssVars(vars)
      const theme = buildDevdrivrTheme(isLight)
      const bg = theme.colors['editor.background']!
