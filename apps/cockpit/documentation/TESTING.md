# TESTING — devdrivr cockpit

> Test strategy, current coverage, and how to add new tests.
...
```bash
# From apps/cockpit/
bun run test          # run once (CI mode)
bun run test:watch    # watch mode (dev)
```

Tests use **Vitest 4** with **jsdom** for DOM simulation and **@testing-library/react** for component tests.
...
### `src/stores/__tests__/notes.store.test.ts` (9 tests) — *only with DB mock*
