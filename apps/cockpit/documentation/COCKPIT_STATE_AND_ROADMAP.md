...
| Unit/component tests | `bunx vitest run` passed: 63 files, 513 tests |
...
| P1       | MCP default-on behavior needs a product/security decision          | `normalizeSettings(null)` enables MCP by default. Even with localhost and bearer auth, a local server starting by default may surprise users. | Decide whether MCP should be opt-in by default. If it remains default-on, make the onboarding/notification explicit.                                                                            | First-run behavior is documented and tested. Settings copy matches actual behavior.                                                        |
| P1       | Platform readiness remains incomplete                              | Docs still mark Windows planned and Linux untested, while bundle config targets all platforms.                                                | Add platform CI/build matrix and smoke-test checklist for macOS, Windows, and Linux. Validate updater artifact naming and manifest coverage.                                                    | Release checklist includes install, launch, persistence, file open/save, updater check, and MCP disabled/enabled smoke tests per platform. |
...
1. Should MCP be enabled by default on first run, or should users explicitly opt in from Settings?
