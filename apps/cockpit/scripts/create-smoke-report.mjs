...
const HELP = `Create an artifact-bound Cockpit release smoke report.
...
async function main() {
  ...
  if (await fileExists(outputPath)) {
    ...
    if (!existingReport.startsWith('# Cockpit Release Smoke Report\n')) {
      throw new Error('--force can replace only an existing Cockpit release smoke report')
    }
  }
