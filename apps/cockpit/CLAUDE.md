...
- **Tests:** `bun run test` (Vitest, 34 tests).
...
### SQLite
All DB access goes through `src/lib/db.ts`. Tables: `settings`, `tool_state`, `notes`, `snippets`, `history`. WAL mode is set at connection time in `getDb()`, not in migrations (SQLite limitation).
