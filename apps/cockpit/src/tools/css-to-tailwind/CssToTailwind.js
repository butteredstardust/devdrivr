...
function convertSizeProperty(prop, value) {
    const prefix = {
...
function convertSpacingProperty(prop, value) {
    const prefix = {
...
export default function CssToTailwind() {
    useMonacoTheme();
    const [state, updateState] = useToolState('css-to-tailwind', {
        input: '',
    });
    ...
    void setLastAction;
    return (_jsx("div", { className: "flex h-full flex-col", children: _jsxs("div", { className: "flex flex-1 overflow-hidden", children: [_jsxs("div", { className: "flex w-1/2 flex-col border-r border-[var(--color-border)]", children: [_jsx("div", { className: "border-b border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-1 text-xs text-[var(--color-text-muted)]", children: "CSS Input" }), _jsx("div", { className: "flex-1", children: _jsx(Editor, { language: "css", value: state.input, onChange: (v) => updateState({ input: v ?? '' }), options: EDITOR_OPTIONS }) })] }), _jsxs("div", { className: "flex w-1/2 flex-col", children: [_jsxs("div", { className: "flex items-center justify-between border-b border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-1", children: [_jsx("span", { className: "text-xs text-[var(--color-text-muted)]", children: "Tailwind Output" }), classString && _jsx(CopyButton, { text: classString })] }), _jsx("div", { className: "flex-1 overflow-auto p-4", children: result ? (_jsxs("div", { className: "flex flex-col gap-4", children: [result.classes.length > 0 && (_jsxs("section", { children: [_jsx("h3", { className: "mb-2 font-pixel text-xs text-[var(--color-success)]", children: "Converted Classes" }), _jsx("div", { className: "flex flex-wrap gap-2", children: result.classes.map((cls, i) => (_jsx("code", { className: "rounded border border-[var(--color-accent)]/30 bg-[var(--color-accent)]/10 px-2 py-0.5 text-xs text-[var(--color-accent)]", children: cls }, i))) }), _jsxs("div", { className: "mt-3 rounded border border-[var(--color-border)] bg-[var(--color-surface)] p-3", children: [_jsx("div", { className: "mb-1 text-xs text-[var(--color-text-muted)]", children: "Full class string:" }), _jsx("code", { className: "text-xs text-[var(--color-text)]", children: classString })] })] })), result.unconvertible.length > 0 && (_jsxs("section", { children: [_jsx("h3", { className: "mb-2 font-pixel text-xs text-[var(--color-warning)]", children: "Unconvertible" }), result.unconvertible.map((prop, i) => (_jsx("div", { className: "text-xs text-[var(--color-text-muted)]", children: prop }, i)))] }))] })) : (_jsx("div", { className: "text-sm text-[var(--color-text-muted)]", children: "Enter CSS on the left to convert" })) })] })] }) }));
}
