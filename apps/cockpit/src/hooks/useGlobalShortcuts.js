...
export function useGlobalShortcuts() {
    ...
    const comboK = useMemo(() => ({ key: 'k', mod: true }), []);
    const comboB = useMemo(() => ({ key: 'b', mod: true }), []);
    const comboShiftN = useMemo(() => ({ key: 'n', mod: true, shift: true }), []);
    const comboShiftT = useMemo(() => ({ key: 't', mod: true, shift: true }), []);
    const comboNext = useMemo(() => ({ key: ']', mod: true }), []);
    const comboPrev = useMemo(() => ({ key: '[', mod: true }), []);
    const comboEnter = useMemo(() => ({ key: 'Enter', mod: true }), []);
    const comboShiftC = useMemo(() => ({ key: 'c', mod: true, shift: true }), []);
    const combo1 = useMemo(() => ({ key: '1', mod: true }), []);
    const combo2 = useMemo(() => ({ key: '2', mod: true }), []);
    const combo3 = useMemo(() => ({ key: '3', mod: true }), []);
    const comboComma = useMemo(() => ({ key: ',', mod: true }), []);
    const comboShiftP = useMemo(() => ({ key: 'p', mod: true, shift: true }), []);
    const comboO = useMemo(() => ({ key: 'o', mod: true }), []);
    const comboS = useMemo(() => ({ key: 's', mod: true }), []);
    const toggleSidebar = useCallback(() => update('sidebarCollapsed', !sidebarCollapsed), [update, sidebarCollapsed]);
    const toggleDrawer = useCallback(() => update('notesDrawerOpen', !notesDrawerOpen), [update, notesDrawerOpen]);
