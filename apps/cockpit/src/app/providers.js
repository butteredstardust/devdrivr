...
export function Providers({ children }) {
    ...
    useEffect(() => {
        async function bootstrap() {
            ...
            const bounds = await getSetting('windowBounds', null);
            if (bounds) {
    ...
    if (error) {
        return (_jsx("div", { className: "flex h-full items-center justify-center p-8", children: _jsxs("div", { className: "text-[var(--color-error)]", children: ["Failed to initialize: ", error] }) }));
    }
    if (!initialized) {
        return (_jsx("div", { className: "flex h-full items-center justify-center", children: _jsx("div", { className: "font-pixel text-sm text-[var(--color-accent)]", children: "Loading..." }) }));
    }
